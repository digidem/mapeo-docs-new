# Tengo problemas de sincronización

Consulte las páginas a continuación para obtener sugerencias de resolución de problemas sobre algunos problemas comunes durante la sincronización:

* [connecting-to-wi-fi.md](connecting-to-wi-fi.md "mention")
* [creating-local-wi-fi-networks](creating-local-wi-fi-networks/ "mention")
  * [con-un-router-portatil.md](creating-local-wi-fi-networks/con-un-router-portatil.md "mention")
  * [con-un-celular.md](creating-local-wi-fi-networks/con-un-celular.md "mention")
* [the-device-i-want-to-sync-with-does-not-show-up-on-my-synchronize-screen.md](the-device-i-want-to-sync-with-does-not-show-up-on-my-synchronize-screen.md "mention")
* [me-aparece-un-error-cuando-intento-sincronizar-con-otro-dispositivo.md](me-aparece-un-error-cuando-intento-sincronizar-con-otro-dispositivo.md "mention")
* [no-puedo-sincronizar-con-el-archivo-de-sincronizacion.md](no-puedo-sincronizar-con-el-archivo-de-sincronizacion.md "mention")
